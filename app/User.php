...
class User extends Authenticatable
{
    ...
    {
        return $this->hasMany(ProjectInstaller::class, 'user_id');
    }
    ...
    {
        return $this->hasOne(DesignerColor::class, 'user_id');
    }
    ...
    public static function allEmployees($exceptId = NULL)
    {
        $users = User::withoutGlobalScope('active')->join('role_user', 'role_user.user_id', '=', 'users.id')
            ...
            ->whereIn('roles.name', ['employee', 'designer', 'installer'])
            ->where('users.id', '<>', '1');
    ...
    public static function allInstallers($exceptId = NULL)
    {
        $users = User::join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->select('users.id', 'users.name', 'users.email', 'users.created_at')
            ->where('roles.name', 'installer');
    ...
    public static function allInstallersList($exceptId = NULL)
    {
        $users = User::join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->where('roles.name', 'installer')
