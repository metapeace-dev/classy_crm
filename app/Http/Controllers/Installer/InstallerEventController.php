...
class InstallerEventController extends InstallerBaseController
{
    ...
    public function index(){
        ...
        return view('installer.event-calendar.index', $this->data);
    }
    ...
    public function edit($id){
        ...
        $view = view('installer.event-calendar.edit', $this->data)->render();
    ...
    public function createPDF($id){
        ...
        if($this->event){
            if($this->event->event_type === 1){
                ...
                $pdf->loadview('installer.event-calendar.event-lead-pdf', $this->data);
                ...
                if(!is_null($this->event->lead_id)){
                    ...
                    $this->designer = User::findorFail($this->event->attendee->user_id);
                    $pdf = app('dompdf.wrapper');
                    $pdf->loadview('installer.event-calendar.event-project-pdf', $this->data);
                }
                else if(!is_null($this->event->client_id)){
                    $this->client = Client::findorFail($this->event->client_id);
                    $this->designer = User::findorFail($this->event->attendee->user_id);
                    $pdf = app('dompdf.wrapper');
                    $pdf->loadview('installer.event-calendar.event-client-pdf', $this->data);
                }
    ...
    public function show($id){
        $this->event = Event::findOrFail($id);
        return view('installer.event-calendar.show', $this->data);
    }
