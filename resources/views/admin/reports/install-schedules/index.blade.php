        ...
        <div class="white-box">
            <div class="row m-b-10">
                ...
                {!! Form::open(['id'=>'report_filter_form','class'=>'ajax-form','method'=>'POST']) !!}
                    ...
                    <div class="col-md-4">
                        <h5 class="box-title m-t-30">Schedule Type</h5>

                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-12">
                                    <select class="select2 form-control" data-placeholder="Schedule Type" id="type_id" name="type_id">
                                        <option value="">All</option>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    @foreach($fieldsData as $key => $value)
                                        @if($value == 'active')
                                            <th>{{ Lang::has('modules.install_schedules.'.$key) ? __('modules.install_schedules.'.$key) : __('modules.install_schedules.'.Illuminate\Support\Str::camel($key)) }}</th>
                                        @endif
                                    @endforeach
...
<script>
    ...
    function showTable() {
        let url = '{!!  route('admin.install-schedule-report.data') !!}';
    ...
    function exportData(){
        let url = '{!!  route('admin.install-schedule-report.export') !!}'+'?'+$('#report_filter_form').serialize();
        window.location.href = url;
    }

    $(document).on('click', '.btn_send_email',function(){
        let id = $(this).data('id');
        var url = '{{ route("admin.install-schedule-report.email", [':id']) }}';
    ...
    function exportPDFData(){
        let url = '{!!  route('admin.install-schedule-report.export-pdf') !!}';
