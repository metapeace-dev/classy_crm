                            ...
                            <thead>
                                <tr>
                                    ...
                                    @foreach($fieldsData as $key => $value)
                                        @if($value == 'active')
                                            <th>{{ Lang::has('modules.client.'.$key) ? __('modules.client.'.$key) : __('modules.client.'.Illuminate\Support\Str::camel($key)) }}</th>
                                        @endif
                                    @endforeach
...
<script>
    ...
    function showTable() {
        let url = '{!!  route('admin.client-report.data') !!}';
    ...
    function exportData(){
        let url = '{!!  route('admin.client-report.export') !!}'+'?'+$('#report_filter_form').serialize();
        window.location.href = url;
    }

    $(document).on('click', '.btn_send_email',function(){
        let id = $(this).data('id');
        var url = '{{ route("admin.client-report.email", [':id']) }}';
    ...
    function exportPDFData(){
        let url = '{!!  route('admin.client-report.export-pdf') !!}';
