...
<div class="modal-body">
    ...
    <div class="form-body">
        ...
        @elseif($event->event_type == 2)
            <div class="form-group" id="update_selectContainer">
                ...
                <div class="radio-list">
                    <label class="radio-inline p-0 col-md-4 text-center">
                        <div class="radio radio-info">
                            <input type="radio" id="show_lead_container_edit" @if(!empty($event->lead_id)) checked @endif name="update-active-container" value="0">
                            <label for="show_lead_container_edit">@lang('modules.events.lead')</label>
                        </div>
                    </label>
                    <label class="radio-inline col-md-4 text-center">
                        <div class="radio radio-info">
                            <input type="radio" id="show_project_container_edit" name="update-active-container" @if(!empty($event->project_id)) checked @endif value="1">
                            <label for="show_project_container_edit">@lang('modules.events.project')</label>
                        </div>
                    </label>
                    <label class="radio-inline col-md-4 text-center">
                        <div class="radio radio-info">
                            <input type="radio" id="show_client_container_edit" name="update-active-container" @if(!empty($event->client_id)) checked @endif value="2">
                            <label for="show_client_container_edit">@lang('modules.events.client')</label>
                        </div>
                    </label>
                </div>
            </div>
            <div class="row" id="lead-container" @if(empty($event->lead_id)) style="display:none;" @endif >
                <div class="col-xs-12">
                    <div class="form-group">
                        <label class="control-label">@lang('modules.module.leads')</label>
                        <select class="select2 form-control" data-placeholder="@lang('modules.events.selectLead')" name="lead_id" id="lead_id" >
            ...
            <div class="row" id="project-container" @if(empty($event->project_id)) style="display:none;" @endif >
                <div class="col-xs-12">
                    <div class="form-group">
            ...
            <div class="row" id="client-container" @if(empty($event->client_id)) style="display:none;" @endif >
                <div class="col-xs-12">
                    <div class="form-group">
                        ...
                        <select class="select2 form-control" data-placeholder="@lang('modules.events.selectClient')" name="client_id" id="client_id" >
                            ...
                            @foreach($clients as $client)
                                <option @if($event->client_id == $client->id) selected @endif value="{{ $client->id }}">{{ $client->full_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function(){
        $('input[name=update-active-container]').click(function () {
            var index = $('input[name=update-active-container]:checked').val();
            if(index === '0'){
                $('#updateEvent #lead-container').show();
                $('#updateEvent #project-container').hide();
                $('#updateEvent #project-container option:selected').removeAttr("selected");
                $('#updateEvent #client-container').hide();
                $('#updateEvent #client-container option:selected').removeAttr("selected");
            }
            else if(index === '1'){
                $('#updateEvent #lead-container').hide();
                $('#updateEvent #lead-container option:selected').removeAttr("selected");
                $('#updateEvent #project-container').show();
                $('#updateEvent #client-container').hide();
                $('#updateEvent #client-container option:selected').removeAttr("selected");
            }
            else{
                ...
                $('#updateEvent #project-container').hide();
                $('#updateEvent #project-container option:selected').removeAttr("selected");
                $('#updateEvent #client-container').show();
            }
        });
    });
