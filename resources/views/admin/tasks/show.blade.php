            ...
            <div class="col-md-12" id="files-list-panel">
                <div class="white-box">
                    ...
                    <div class="row m-b-20 hide" id="file-dropzone">
                        <div class="col-md-12">
                            <form action="{{ route('admin.task-files.storeFile') }}" class="dropzone"
    ...
    $('#reminderButton').click(function () {
        swal({
            ...
            if (isConfirm) {
                ...
                var url = '{{ route('admin.all-tasks.email', $task->id)}}';
   ...
   function markComplete(status) {
    ...
    $('body').on('click', '.delete-file', function () {
        ...
        swal({
            ...
            if (isConfirm) {
                var url = "{{ route('admin.task-files.deleteFile') }}";
                var token = "{{ csrf_token() }}";
    ...
    $('.thumbnail').on('click', function(event) {
        ...
        $.easyAjax({
            type: 'GET',
            url: "{{ route('admin.task-files.thumbnail') }}",
            data: {
                id: taskID
            },
            success: function (response) {
    ...
    $('#save-link').click(function () {
        $.easyAjax({
            url: '{{route('admin.task-files.storeLink')}}',
